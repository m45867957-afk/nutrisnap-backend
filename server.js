...
// تا کاربر رایگان به‌خاطر یک عکس اشتباه (مثلاً گربه یا کتاب)، یکی از ۳ اسکن هفتگی‌اش را از دست ندهد.
async function refundWeeklyCounter(key) {
  const week = isoWeekKey();
  const entry = await kvGet(key);
  if (entry && entry.week === week && entry.count > 0) {
    entry.count -= 1;
    await kvSet(key, entry, 60 * 60 * 24 * 10);
  }
}
...
async function refundFreeUsage(userId, ip) {
  await refundWeeklyCounter(`usage:${userId}`);
  if (ip) await refundWeeklyCounter(`usage_ip:${ip}`);
}
