...
async function checkAndConsumeFreeUsage(userId) {
  const week = isoWeekKey();
  const key = `usage:${userId}`;
  const entry = await kvGet(key);
  if (!entry || entry.week !== week) {
    await kvSet(key, { week, count: 1 }, 60 * 60 * 24 * 10);
    return { allowed: true, remaining: FREE_WEEKLY_LIMIT - 1 };
  }
  if (entry.count >= FREE_WEEKLY_LIMIT) {
    return { allowed: false, remaining: 0 };
  }
  entry.count += 1;
  await kvSet(key, entry, 60 * 60 * 24 * 10);
  return { allowed: true, remaining: FREE_WEEKLY_LIMIT - entry.count };
}
...
app.post("/analyze", analyzeLimiter, async (req, res) => {
  ...
  try {
    ...
    let usageInfo = null;
    if (!premium) {
      const usage = await checkAndConsumeFreeUsage(userId);
      if (!usage.allowed) {
