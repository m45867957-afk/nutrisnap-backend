...
app.post("/admin/grant-premium", express.json({ limit: "10kb" }), async (req, res) => {
...
app.post("/admin/revoke-premium", express.json({ limit: "10kb" }), async (req, res) => {
  const { userId, secret } = req.body;
  if (!process.env.ADMIN_SECRET || secret !== process.env.ADMIN_SECRET) {
    return res.status(403).json({ error: "دسترسی مجاز نیست" });
  }
...
app.post("/admin/resync-subscriptions", express.json({ limit: "10kb" }), async (req, res) => {
