...
// این خط باید همیشه اولین require در کل فایل باشد — قبل از express و هر پکیج دیگه‌ای —
// چون Sentry برای رصد درست خطاها باید قبل از بقیه‌ی ماژول‌ها راه‌اندازی شود.
...
// چند هدر امنیتی پایه (جلوگیری از حملات رایج مثل clickjacking، sniffing نوع فایل و غیره).
// contentSecurityPolicy را خاموش می‌کنیم چون این سرور فقط API می‌دهد، صفحه‌ی HTML رندر نمی‌کند.
...
const ALLOWED_ORIGINS = [
  "https://animated-cactus-4e3aa9.netlify.app",
  process.env.FRONTEND_URL, // می‌توانی این متغیر را در Railway ست کنی تا نیازی به ویرایش کد نباشد
].filter(Boolean);

// استثنای origin "null" (یعنی صفحه‌ای که مستقیم از روی فایل file:// باز شده، مثل admin-tool.html)
// فقط برای مسیرهای /admin/* مجاز است، نه برای کل سایت — چون این استثنا در اصل فقط برای
// همان ابزار مدیریت محلی لازم بود؛ باز گذاشتنش برای همه‌ی مسیرها یعنی هر فایل HTML لوکال دیگری
// هم می‌تواند به /analyze یا /create-checkout-session وصل شود، که ربطی به هدف اصلی‌اش ندارد.
const corsForAdmin = cors({
...
const corsForEveryoneElse = cors({
  origin: (origin, callback) => {
...
app.use("/admin", corsForAdmin);
// برای مسیرهای غیر از /admin، همان محدودیت سخت‌گیرانه اجرا می‌شود؛ مسیرهای /admin چون بالا
// از قبل CORS خودشان را گرفته‌اند، اینجا رد می‌شوند تا دوباره (این‌بار سخت‌گیرانه) بررسی نشوند.
app.use((req, res, next) => {
  if (req.path.startsWith("/admin")) return next();
  corsForEveryoneElse(req, res, next);
});
app.set("trust proxy", 1);

// از «۳ اسکن در هفته» به «۱ اسکن در روز» تغییر کرد: هدف این است که کاربر رایگان هر روز
// با اپ سر و کار داشته باشد (نزدیک‌تر به عادت روزانه‌ای که کل ارزش پیشنهادی محصول است)
// به‌جای اینکه سهمیه‌اش خیلی زود در وسط هفته تمام شود و اپ را فراموش کند.
const FREE_DAILY_LIMIT = 1;

function isoDayKey(date = new Date()) {
  const d = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()));
  return d.toISOString().slice(0, 10); // مثلاً "2026-09-21"
}
...
const FREE_DAILY_LIMIT_PER_IP = 4;

async function consumeDailyCounter(key, limit) {
  const day = isoDayKey();
  const entry = await kvGet(key);
  if (!entry || entry.day !== day) {
    await kvSet(key, { day, count: 1 }, 60 * 60 * 24 * 3);
  ...
  entry.count += 1;
  await kvSet(key, entry, 60 * 60 * 24 * 3);
  return { allowed: true, remaining: limit - entry.count };
}

// وقتی عکس اصلاً غذا نبوده (isFood: false)، سهم اسکن رایگانی که همین الان مصرف شده را برمی‌گردانیم
// تا کاربر رایگان به‌خاطر یک عکس اشتباه (مثلاً گربه یا کتاب)، اسکن روزانه‌اش را از دست ندهد.
async function refundDailyCounter(key) {
  const day = isoDayKey();
  const entry = await kvGet(key);
...
async function refundFreeUsage(userId, ip) {
  await refundDailyCounter(`usage:${userId}`);
  if (ip) await refundDailyCounter(`usage_ip:${ip}`);
}

async function checkAndConsumeFreeUsage(userId, ip) {
  const byUser = await consumeDailyCounter(`usage:${userId}`, FREE_DAILY_LIMIT);
  const byIp = ip ? await consumeDailyCounter(`usage_ip:${ip}`, FREE_DAILY_LIMIT_PER_IP) : { allowed: true, remaining: FREE_DAILY_LIMIT };
...
async function getRemainingFreeUsage(userId) {
  ...
  if (!entry || entry.day !== day) return FREE_DAILY_LIMIT;
...
// ---------- توکن نشست (Session Token) ----------
// چون userId از روی فرمول عمومی sha256(ایمیل) ساخته می‌شود، به‌تنهایی یک "رمز" محسوب نمی‌شود —
// هرکس ایمیل یک نفر را بداند، می‌تواند همان userId را offline حساب کند و جای او وانمود کند.
// برای همین، بعد از تأیید موفق کد OTP، یک توکن امضاشده (HMAC) صادر می‌شود که فقط خود سرور
// می‌تواند بسازد؛ از این پس مسیرهای حساس این توکن را می‌خواهند، نه صرفاً یک userId خام.
const SESSION_SECRET = process.env.SESSION_SECRET || null;
if (!SESSION_SECRET) {
  console.warn("⚠️ SESSION_SECRET تنظیم نشده — توکن‌های نشست امضا نمی‌شوند و احراز هویت غیرفعال است!");
}
const SESSION_TTL_SECONDS = 60 * 60 * 24 * 180; // ۱۸۰ روز
...
function verifySessionToken(token) {
  ...
  if (Date.now() / 1000 > exp) return null; // منقضی‌شده

  const expected = crypto.createHmac("sha256", SESSION_SECRET).update(`${userId}.${expStr}`).digest();
...
// این میدل‌ور را روی هر مسیری بگذار که باید مطمئن باشی درخواست واقعاً از طرف همان کاربر تأییدشده است.
// هدر Authorization: Bearer <token> را می‌خواند و req.authUserId را ست می‌کند.
function requireAuth(req, res, next) {
  const header = req.headers.authorization || "";
  const token = header.startsWith("Bearer ") ? header.slice(7) : null;
  const userId = token && verifySessionToken(token);
  if (!userId) {
    return res.status(401).json({ error: "unauthorized", message: "لطفاً دوباره وارد شو." });
  }
...
async function sendOtpEmail(email, code, lang) {
  ...
  const isFa = lang === "fa";
  const subject = isFa ? `${code} — کد تأیید NutriSnap` : `${code} — Your NutriSnap verification code`;
  const html = isFa
...
const authSendLimiter = rateLimit({
  ...
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: "too_many_requests", message: "Too many requests, please wait a few minutes." },
});
...
// مسیرهای /admin/* اجازه‌ی تغییر وضعیت پرداخت/اشتراک کاربرها را می‌دهند — باید در برابر
// حدس‌زدن ADMIN_SECRET با تلاش‌های پیاپی محافظت جدی‌تری نسبت به بقیه‌ی مسیرها داشته باشند.
const adminLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 5, // حداکثر ۵ تلاش در هر ۱۵ دقیقه برای هر IP
  standardHeaders: true,
  legacyHeaders: false,
  message: { error: "too_many_requests", message: "Too many attempts, please wait before trying again." },
});

// مقایسه‌ی معمولی رشته‌ها (===) می‌تواند نشت زمانی (timing attack) داشته باشد — یعنی مهاجم
// می‌تواند از روی مدت‌زمان پاسخ سرور حدس بزند کدام کاراکترهای رمز درست بوده‌اند.
// crypto.timingSafeEqual این نشت را از بین می‌برد؛ باید هر دو رشته هم‌طول باشند، برای همین
// اول طول‌ها را چک می‌کنیم (این چک خودش وابسته به مقدار رمز نیست، پس امن است).
...
app.post("/auth/send-code", authSendLimiter, express.json({ limit: "10kb" }), async (req, res) => {
  try {
    ...
    // crypto.randomInt به‌جای Math.random(): برای یک کد امنیتی، تصادفی‌سازی رمزنگارانه لازم است
    // (Math.random قابل‌پیش‌بینی‌تر است و برای اهداف امنیتی مناسب نیست).
    const code = String(crypto.randomInt(100000, 1000000)); // کد ۶ رقمی
    ...
    res.status(500).json({ error: "send_failed", message: "Couldn't send the code." });
  }
});

app.post("/auth/verify-code", authVerifyLimiter, express.json({ limit: "10kb" }), async (req, res) => {
  try {
    const email = (req.body.email || "").trim().toLowerCase();
  ...
  async (req, res) => {
    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        // شناسه‌ی کاربر را روی خود مشتری در Stripe هم ثبت می‌کنیم (metadata).
        // این کار باعث می‌شود Stripe خودش نسخه‌ی همیشگی این نگاشت را نگه دارد —
        // اگر یک روز داده‌های Redis از بین برفت، می‌شود از همین‌جا (نه از Redis) دوباره بازسازی‌اش کرد.
        try {
          ...
          console.error("ثبت userId روی متادیتای مشتری Stripe ناموفق بود:", metaErr.message);
...
// محدودیت حجم درخواست — بدون این، هرکسی می‌تونه یه فایل خیلی بزرگ بفرسته و سرور رو کند/خراب کنه.
// ۱۰ مگابایت برای عکس‌های base64 (که حجمشون معمولاً ۳۳٪ بیشتر از فایل اصلیه) کافی و منطقیه.
...
app.post("/create-checkout-session", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const userId = req.authUserId; // هرگز از ورودی خام کلاینت اعتماد نکن — از توکن تأییدشده بگیر
    const priceId =
...
app.get("/subscription-status/:userId", requireAuth, async (req, res) => {
  if (req.params.userId !== req.authUserId) {
    return res.status(403).json({ error: "forbidden" });
  }
  const sub = await kvGet(`sub:${req.authUserId}`);
  const premium = await isPremium(req.authUserId);
  res.json({
    status: sub ? sub.status : "none",
    plan: sub ? sub.plan : null,
    isPremium: premium,
    freeRemaining: premium ? null : await getRemainingFreeUsage(req.authUserId),
    freeLimit: FREE_DAILY_LIMIT,
  });
});

app.get("/usage/:userId", requireAuth, async (req, res) => {
  if (req.params.userId !== req.authUserId) {
    return res.status(403).json({ error: "forbidden" });
  }
  const premium = await isPremium(req.authUserId);
  res.json({
    isPremium: premium,
    remaining: premium ? null : await getRemainingFreeUsage(req.authUserId),
    limit: FREE_DAILY_LIMIT,
  });
});

// این دو اندپوینت با POST کار می‌کنند (نه GET) چون آدرس‌های GET معمولاً در لاگ سرور،
// تاریخچه‌ی مرورگر، و پراکسی‌های میانی ذخیره می‌شوند — یعنی ADMIN_SECRET می‌توانست لو برود.
app.post("/admin/grant-premium", adminLimiter, express.json({ limit: "10kb" }), async (req, res) => {
...
app.post("/admin/revoke-premium", adminLimiter, express.json({ limit: "10kb" }), async (req, res) => {
  const { userId, secret } = req.body;
  if (!isValidAdminSecret(secret)) {
    return res.status(403).json({ error: "دسترسی مجاز نیست" });
  }
...
// بازسازی داده‌های اشتراک در Redis مستقیم از روی Stripe (نسخه‌ی «بک‌آپ» بدون هیچ سرویس یا هزینه‌ی اضافه).
// Stripe همیشه منبع اصلی حقیقت برای وضعیت پرداخت‌هاست؛ این مسیر فقط sub:* و cust2user:* را
// در Redis از نو می‌سازد — برای وقتی که مثلاً Redis خالی شده یا داده‌هایش خراب شده.
// فقط مشتری‌هایی که بعد از فعال شدن این قابلیت پرداخت کرده‌اند، متادیتای userId دارند و قابل بازسازی‌اند؛
// نتیجه‌ی درخواست، لیست مشتری‌های بدون این متادیتا را هم نشان می‌دهد تا بدانی کدام‌ها دستی نیاز به بررسی دارند.
app.post("/admin/resync-subscriptions", adminLimiter, express.json({ limit: "10kb" }), async (req, res) => {
  const { secret } = req.body;
  if (!isValidAdminSecret(secret)) {
    return res.status(403).json({ error: "دسترسی مجاز نیست" });
  }
  if (!stripe) {
    return res.status(503).json({ error: "Stripe تنظیم نشده است" });
  }
  ...
  try {
    for (const status of ["active", "trialing", "past_due"]) {
      ...
      while (true) {
        ...
        for (const sub of page.data) {
          ...
          const userId = customer && customer.metadata && customer.metadata.userId;
          if (!userId) {
            skippedNoUserId.push({ customerId: customer && customer.id, subscriptionId: sub.id });
            continue;
          }
          const plan = sub.items && sub.items.data && sub.items.data[0]
            ? sub.items.data[0].price.id
            : null;
          await kvSet(`sub:${userId}`, { status: sub.status, customerId: customer.id, plan });
          await kvSet(`cust2user:${customer.id}`, userId);
          restored.push({ userId, customerId: customer.id, status: sub.status });
        }
    ...
    res.json({
      message: `${restored.length} اشتراک در Redis بازسازی شد.`,
      restoredCount: restored.length,
      restored,
      skippedNoUserIdCount: skippedNoUserId.length,
      skippedNoUserId,
    });
  } catch (err) {
    console.error("بازسازی اشتراک‌ها از Stripe ناموفق بود:", err.message);
    Sentry.captureException(err);
    res.status(500).json({ error: "بازسازی ناموفق بود" });
  }
});
...
app.post("/analyze", analyzeLimiter, requireAuth, async (req, res) => {
  ...
  try {
    ...
    const userId = req.authUserId; // هرگز از ورودی خام کلاینت اعتماد نکن — از توکن تأییدشده بگیر
    if (!base64) {
      return res.status(400).json({ error: "base64 لازم است" });
    }
    ...
    if (!premium) {
      ...
      if (!usage.allowed) {
        return res.status(403).json({
          ...
          message:
            lang === "en"
              ? "You've used today's free analysis. Upgrade to Premium for unlimited access."
              : "تحلیل رایگان امروزت تموم شد. برای دسترسی نامحدود، Premium رو فعال کن.",
        });
      }
      usageInfo = usage;
    }
...
"recipe": "one short recipe or diet suggestion using this food, 1-2 sentences"}`
        : `اول بررسی کن که این عکس واقعاً یک ماده‌ی خوراکی (میوه، سبزی، غذای پخته، نوشیدنی یا محصول بسته‌بندی‌شده) نشان می‌دهد یا نه — ممکن است یک غذای ایرانی باشد. اگر عکس چیزی خوراکی نشان نمی‌دهد (مثلاً یک شخص، یک شیء، حیوان خانگی، سند، یا اسکرین‌شات)، فقط همین JSON را برگردان و متوقف شو: {"isFood": false}. در غیر این صورت، قبل از نام‌گذاری با دقت زیاد نگاه کن — دقت خیلی مهم است، چون تشخیص اشتباه کل اعداد تغذیه‌ای را بی‌اعتبار می‌کند. اگر کاملاً مطمئن نیستی، نزدیک‌ترین غذای رایج را انتخاب کن و مقدار confidencePercent را پایین‌تر بگذار، نه اینکه حدس تصادفی بزنی. آن را شناسایی کن، یک نام ساده و رایج انگلیسی برای جستجو در دیتابیس بده، وزن قابل مشاهده در تصویر را به گرم تخمین بزن، و مقادیر تغذیه‌ای را برای همان وزن تخمینی محاسبه کن. همچنین مناسب بودنش را برای سه گروه ارزیابی کن، یک جمع‌بندی کوتاه یک‌خطی بنویس، یک پیشنهاد عملی و کاربردی با عدد واقعی بده، یک جایگزین سالم‌تر پیشنهاد بده، و یک پیشنهاد کوتاه دستور غذا یا رژیم مرتبط هم بده. فقط یک شیء JSON خام برگردان، بدون هیچ توضیح یا Markdown، دقیقاً با این ساختار:
{"isFood": true, "name": "نام فارسی ماده خوراکی", "searchTerm": "نام ساده انگلیسی ماده غذایی", "confidence": "high" یا "medium" یا "low", "confidencePercent": عددی بین ۰ تا ۱۰۰, "weightGrams": عدد, "serving": "مثلاً یک عدد متوسط تقریباً ۱۵۰ گرم",
"calories": عدد, "protein": عدد, "carbs": عدد (کربوهیدرات به گرم), "sugar": عدد, "fiber": عدد, "fat": عدد, "calcium": عدد, "iron": عدد, "potassium": عدد, "vitaminC": عدد, "vitaminA": عدد,
...
"recipe": "یک پیشنهاد کوتاه دستور غذا یا رژیم با این ماده، در ۱ تا ۲ جمله"}`;
    ...
    if (parsed.isFood === false) {
      // این یکی از ۳ اسکن رایگان هفتگی محسوب نشود — عکس اصلاً غذا نبوده.
      if (!premium) {
        await refundFreeUsage(userId, req.ip);
      }
      return res.json({
        isFood: false,
        message:
          lang === "en"
            ...
            : "این عکس یک ماده‌ی خوراکی به نظر نمی‌رسد. لطفاً عکس واضح‌تری از غذا بگیر.",
      });
    }
...
app.post("/meal-plan", requireAuth, async (req, res) => {
  try {
    const { recentFoods, lang } = req.body;
    const userId = req.authUserId; // هرگز از ورودی خام کلاینت اعتماد نکن — از توکن تأییدشده بگیر
...
// این میدل‌ور باید بعد از همه‌ی route ها و قبل از app.listen باشد تا خطاهای هر اندپوینت
// را خودکار به Sentry گزارش کند (اگر SENTRY_DSN تنظیم شده باشد).
